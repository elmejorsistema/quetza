...
switch($universo)
  ...
  case "grupo";
   ...
   $q="insert into concentrado select a.id, a.1_apellido, a.2_apellido, a.nombre, m.id, m.name, ahe.calif_5, m.secuencia_2, a.capacitacion_id from alumno_has_evaluacion as ahe join ciclo_has_materia as chm on chm.id = ahe.ciclo_has_materia_id join alumno as a on a.id = ahe.alumno_id join materia as m on chm.materia_id = m.id where chm.ciclo_id = '$o_config->ciclo' and  chm.ciclo_tipo = '$o_config->tipo' and chm.grupo_id = $grupo and ((chm.materia_id >= 1 and chm.materia_id <= 95) or (chm.materia_id >= 106  and chm.materia_id <= 113) or chm.materia_id >= 115)";
...
$q = "insert into concentrado_has_resumen select alumno_id, 0, round(avg(calif),4) from concentrado group by alumno_id"; 
$o_database->query_assign($q);
...
/* Ya no se redondea la calificación */
$q = "select c.alumno_id, concat(concat(concat(concat(c.1_apellido, \" \"), c.2_apellido), \" \"), c.nombre), c.calif, c.materia_id, c.materia_secuencia, chr.promedio, capacitacion_id from concentrado as c join concentrado_has_resumen as chr on c.alumno_id = chr.alumno_id where chr.reprobadas >= $reprobada1 and chr.reprobadas <= $reprobada2  $order"; 
//echo $q;return;
...
$numero_total_reg     = $o_database->query_num_rows/$total_materias;
$contador_alumnos     = 0;
//echo $numero_total_reg; return;
...
while($row = mysql_fetch_row($result))
  {
    ...
    $row[2] = ($row[2] == 10) ? $row[2] = number_format($row[2]) : $row[2];
	...
	if($contador_registros > $num_registros)
	  {
        ...
        if($numero_total_reg != $contador_alumnos){
           $html .= "</tbody></table>";
           // echo $numero_total_reg. "---".  $contador_alumnos;
        }
        $cerro_tabla = true;
...
foreach($a_materias as $key => $value)
  {
    ...
    $html.= "<td class=\"$class\">".number_format($promedio,4)."</td>";
  }


if($contador > 0)
  $promedio = round($promedio_g/$contador,4);
else
  $promedio = 0;
...
$html.= "<td class=\"$class\">".number_format($promedio,4)."</td></tr>";
